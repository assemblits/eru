...
public class OneEightyGaugeSkin extends SkinBase<OneEightyGauge> implements Skin<OneEightyGauge> {
    ...
    // ******************** Constructors **************************************
    public OneEightyGaugeSkin(){
        super(new OneEightyGauge());
    }

    public OneEightyGaugeSkin(OneEightyGauge gauge) {
        super(gauge);
        currentValue      = new SimpleDoubleProperty(this, "currentValue", 0);
    ...
    private void resize() {
        ...
        if (width > 0 && height > 0) {
            ...
            minText.setText(String.format(Locale.US, "%." + getSkinnable().getDecimals() + "f", getSkinnable().getMinValue()));
            minText.relocate(((0.27778 * width) - minText.getLayoutBounds().getWidth()) * 0.5, 0.7 * height);

            maxText.setFont(smallFont);
            maxText.setText(String.format(Locale.US, "%." + getSkinnable().getDecimals() + "f", getSkinnable().getMaxValue()));
