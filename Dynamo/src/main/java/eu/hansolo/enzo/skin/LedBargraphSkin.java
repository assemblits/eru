...
import eu.hansolo.enzo.LedBargraph;
...
public class LedBargraphSkin extends SkinBase<LedBargraph> implements Skin<LedBargraph> {
    ...
    // ******************** Constructors **************************************
    public LedBargraphSkin(){
        super(new LedBargraph());
    }
    ...
    private void init() {
        ...
        for(int i = 0 ; i < getSkinnable().getNoOfLeds() ; i++) {
            ...
            led.setPrefWidth(getSkinnable().getLedSize());
            led.setPrefHeight(getSkinnable().getLedSize());
            ledList.add(led);
